...
class PostgresUserRepository(UserRepository):
    ...
    def list_all(self) -> list[UserEntity]:
        ...
        return [UserEntity(id=user.id, username=user.username, email=user.email, password=user.hashed_password) for user in db_users]
...
class PostgresVideoRepository(VideoRepository):
    ...
    def get_by_id(self, video_id: int) -> Optional[VideoEntity]:
