...
class VideoPersistenceAdapterImplementationTest {
    ...
    @Test
    void save_verifyEntityMapping() {
        when(jpaRepository.save(any(VideoEntity.class))).thenReturn(videoEntity);
        
        videoPersistenceAdapter.save(video);
    ...
    @Test
    void mapToDomain_withNullEntity_throwsNullPointerException() {
        assertThrows(NullPointerException.class, () -> {
            videoPersistenceAdapter.mapToDomain(null);
        });
    }

    @Test
    void mapToDomain_withCompleteEntity() {
        Video result = videoPersistenceAdapter.mapToDomain(videoEntity);
        
        assertNotNull(result);
        assertEquals(videoEntity.getId(), result.getId());
        assertEquals(videoEntity.getTitle(), result.getTitle());
        assertEquals(videoEntity.getOriginalFileName(), result.getOriginalFileName());
        assertEquals(videoEntity.getStoragePath(), result.getStoragePath());
        assertEquals(videoEntity.getStatus(), result.getStatus());
        assertEquals(videoEntity.getUsername(), result.getUsername());
        assertEquals(videoEntity.getUserEmail(), result.getUserEmail());
        assertEquals(videoEntity.getContentType(), result.getContentType());
        assertEquals(videoEntity.getCreatedAt(), result.getCreatedAt());
    }

    @Test
    void mapToDomain_withEntityWithNullFields() {
        VideoEntity entityWithNulls = VideoEntity.builder()
                .id(videoId)
                .title("Test Video")
                .originalFileName("test.mp4")
        ...
        Video result = videoPersistenceAdapter.mapToDomain(entityWithNulls);
    ...
    @Test
    void save_withRepositoryException() {
        when(jpaRepository.save(any(VideoEntity.class))).thenThrow(new RuntimeException("Database error"));
    ...
    @Test
    void findById_withRepositoryException() {
        when(jpaRepository.findById(videoId)).thenThrow(new RuntimeException("Database error"));
    ...
    @Test
    void findByUsername_withRepositoryException() {
        when(jpaRepository.findByUsername(username)).thenThrow(new RuntimeException("Database error"));
        
        assertThrows(RuntimeException.class, () -> {
            videoPersistenceAdapter.findByUsername(username);
        });
